...
export function DocsHeader({ onOpenNavigation, onOpenSearch }: DocsHeaderProps) {
  return (
    <header className="sticky top-0 z-30 border-b border-border bg-background/95 backdrop-blur">
      <a
        className="fixed top-3 left-3 z-[80] -translate-y-24 border border-foreground bg-background px-3 py-2 text-xs text-foreground no-underline focus:translate-y-0"
        href="#docs-content"
      >
        Skip to content
      </a>
      <div className="mx-auto flex h-14 max-w-[1600px] items-center px-3 sm:px-5">
        ...
        <Link
          className="flex min-w-0 items-center gap-2 text-sm font-semibold text-foreground no-underline"
          href="/docs"
          aria-label="tynt docs"
        >
        ...
        >
          ...
          <kbd className="ml-auto hidden border border-current px-1.5 py-0.5 text-[10px] md:inline">
            ⌘ K
          </kbd>
        </button>
        ...
        <div className="ml-2 hidden border-y border-r border-border md:flex">
          <a
            ...
            href="https://tynt.dev/gallery"
            aria-label="Open the tynt gallery"
          >
            <Gamepad width={18} height={18} aria-hidden="true" />
          </a>
          <a
            className={iconLinkClass}
            href="https://github.com/cristianbgp/tynt"
            aria-label="View tynt on GitHub"
          >
