...
describe("custom MDX components", () => {
  test("renders semantic callouts, steps, and link cards", () => {
    render(
      <>
        <Callout title="Keep it safe" variant="warning">
          Review the source.
        </Callout>
        <Steps>
          <h3>Make a cartridge</h3>
          <p>Start small.</p>
        </Steps>
        <LinkCard href="https://tynt.dev" title="Open editor">
          Build in the browser.
        </LinkCard>
      </>,
    );

    expect(screen.getByRole("note", { name: "Keep it safe" })).toHaveAttribute(
      "data-variant",
      "warning",
    );
