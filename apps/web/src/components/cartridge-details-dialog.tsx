...
export function CartridgeDetailsDialog({
  open,
  draft,
  onOpenChange,
  onSave,
}: CartridgeDetailsDialogProps) {
      ...
      <DialogContent aria-describedby="cartridge-details-description">
        <div className="dialog-heading flex min-h-[64px] items-stretch justify-between border-b border-border">
          <div className="grid content-center gap-[4px] px-[16px] py-[10px]">
            ...
            <DialogDescription
              ...
              id="cartridge-details-description"
            >
              tynt cartridge · format v1 · all fields required to export
            </DialogDescription>
          </div>
          <DialogClose asChild>
        ...
        <form className="details-form grid gap-[16px] p-[18px]" onSubmit={submit}>
          <label className="grid gap-[7px] text-[11px]">
            <span>Title</span>
            <input
              className="w-full border border-border bg-background px-[12px] py-[10px] font-[inherit] text-foreground outline-none focus:border-foreground focus:shadow-[inset_0_0_0_1px_var(--foreground)]"
              autoFocus
              value={fields.title}
          ...
          <label className="grid gap-[7px] text-[11px]">
            ...
            <textarea
              className="min-h-[88px] w-full resize-y border border-border bg-background px-[12px] py-[10px] font-[inherit] text-foreground outline-none focus:border-foreground focus:shadow-[inset_0_0_0_1px_var(--foreground)]"
              value={fields.description ?? ""}
              onChange={(event) => update("description", event.target.value)}
              placeholder="What is this cartridge?"
            />
          </label>
          <label className="grid gap-[7px] text-[11px]">
            <span>Controls</span>
            <input
              ...
              onChange={(event) => update("controls", event.target.value)}
              placeholder="Arrows move · A jumps"
            />
          </label>
          {error ? (
            <p className="form-error m-0 text-[11px] text-[var(--error)]" role="alert">
              {error}
            </p>
          ) : null}
          <div className="dialog-actions -mx-[18px] mt-[2px] -mb-[18px] flex justify-end border-t border-border">
            <DialogClose asChild>
