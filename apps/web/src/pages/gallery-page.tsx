        ...
        <section className="cartridge-grid mx-auto grid max-w-[1180px] grid-cols-2 border-t border-l border-border max-[900px]:grid-cols-1" aria-label="Public cartridges">
          {filtered.map((cartridge, index) => (
            <article className="cartridge-card grid min-w-0 grid-cols-[minmax(160px,2fr)_minmax(180px,3fr)] border-r border-b border-border data-[featured=true]:col-span-full data-[featured=true]:grid-cols-[minmax(260px,3fr)_minmax(260px,2fr)] max-[900px]:grid-cols-1 max-[900px]:data-[featured=true]:col-auto max-[900px]:data-[featured=true]:grid-cols-1" data-featured={index === 0 && !query && !tag || undefined} key={cartridge.slug}>
              <Link className="cartridge-artwork grid min-h-[230px] place-items-center bg-muted p-[28px] no-underline hover:bg-[#e5e5e5] focus-visible:bg-[#e5e5e5] max-[560px]:min-h-[172px] max-[560px]:p-[12px]" aria-label={`View ${cartridge.title} details`} to={`/cartridges/${cartridge.slug}`} {...soundLinkProps}><CartridgePreview filename={cartridge.filename} src={cartridge.coverUrl} /></Link>
              <div className="cartridge-copy flex min-w-0 flex-col border-l border-border p-[22px] [overflow-wrap:anywhere] max-[900px]:border-t max-[900px]:border-l-0">
                <div className="cartridge-title-row flex items-baseline justify-between gap-[16px]"><h2 className="m-0 text-[16px] font-semibold tracking-[-0.03em]"><Link className="border-b border-transparent no-underline hover:border-current focus-visible:border-current" to={`/cartridges/${cartridge.slug}`} {...soundLinkProps}>{cartridge.filename}</Link></h2><span className="text-[10px] text-muted-foreground">{String(index + 1).padStart(2, "0")}</span></div>
