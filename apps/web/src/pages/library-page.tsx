...
function searchableText(record: LibraryCartridge): string {
  return [record.title, record.author, record.description, record.controls]
    .filter(Boolean)
    .join(" ")
    .toLowerCase();
}
    ...
    <div className="gallery-shell library-shell grid h-full w-full grid-rows-[41px_minmax(0,1fr)_25px] bg-background max-[560px]:h-dvh max-[560px]:grid-rows-[41px_minmax(0,1fr)_calc(41px+env(safe-area-inset-bottom))]">
      ...
      <main className="gallery-main overflow-auto px-[clamp(18px,5vw,72px)] pt-[42px] pb-[72px] max-[560px]:px-[14px] max-[560px]:pt-[28px] max-[560px]:pb-[28px]">
        <header className="library-heading mx-auto mb-[32px] flex max-w-[1180px] items-end justify-between gap-[28px] border-b border-foreground pb-[24px] max-[560px]:flex-col max-[560px]:items-stretch">
          <div>
            <p className="eyebrow m-0 mb-[9px] text-[10px] text-muted-foreground uppercase">
        ...
        {error ? (
          <p
            className="library-error mx-auto max-w-[1180px] border border-[var(--error)] p-[20px] text-[var(--error)]"
            role="alert"
          >
            {error}
          </p>
        ...
        {!loading && records.length > 0 && filtered.length === 0 ? (
          <p className="library-no-results mx-auto max-w-[1180px] border border-border p-[20px]">
            No cartridges match “{query}”.
          </p>
        ) : null}
        <section
          className="library-grid mx-auto grid max-w-[1180px] grid-cols-2 border-t border-l border-border max-[900px]:grid-cols-1"
          aria-label="Saved cartridges"
        >
          {filtered.map((record) => (
            <article
              className="library-card grid min-w-0 grid-cols-[minmax(150px,2fr)_minmax(190px,3fr)] grid-rows-[minmax(220px,auto)_41px] border-r border-b border-border [overflow-wrap:anywhere] max-[560px]:grid-cols-1 max-[560px]:grid-rows-[auto_auto_41px]"
              key={record.id}
            >
              <div className="library-thumbnail grid min-w-0 place-items-center bg-muted p-[24px]">
                {record.thumbnail ? (
                  <img
                    className="w-[min(100%,320px)] border border-foreground bg-foreground [image-rendering:pixelated]"
                    src={record.thumbnail}
                    alt={`Latest ${record.title} preview`}
                  />
                ) : (
                  <span
              ...
              <div className="library-card-copy flex min-w-0 flex-col border-l border-border p-[20px] [overflow-wrap:anywhere] max-[560px]:border-t max-[560px]:border-l-0">
                ...
                {record.author ? (
                  <p className="library-author mt-[6px] mb-0 text-[10px] text-muted-foreground">
                    by {record.author}
                  </p>
                ) : null}
                <p className="my-[16px] leading-normal text-[#555555]">
                  {record.description || "No description yet."}
                </p>
                <span className="cartridge-controls text-[10px] leading-normal text-muted-foreground">
                  {record.controls || "Controls not documented"}
                </span>
                <time
                  className="mt-auto pt-[16px] text-[10px] text-muted-foreground"
              ...
              <div className="library-card-actions col-span-full grid grid-cols-5 border-t border-border max-[560px]:[&_span]:hidden [&>a]:flex [&>a]:min-w-0 [&>a]:items-center [&>a]:justify-center [&>a]:gap-[7px] [&>a]:border-l [&>a]:border-border [&>a]:px-[10px] [&>a]:no-underline [&>a:focus-visible]:bg-foreground [&>a:focus-visible]:text-background [&>a:hover]:bg-foreground [&>a:hover]:text-background [&>button]:min-w-0 [&>button]:px-[10px]">
                ...
                <Button
                  aria-label={`Edit details for ${record.title}`}
                  onClick={() => setEditing(record)}
                >
                  <InfoBox width={24} height={24} aria-hidden="true" />
                  <span>Details</span>
                </Button>
                <Button
                  aria-label={`Duplicate ${record.title}`}
                  onClick={() => {
                    void duplicate(record);
                  }}
                >
      ...
      <SiteFooter
        summary={`${records.length} local ${records.length === 1 ? "cartridge" : "cartridges"}`}
