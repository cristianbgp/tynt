...
function harness({ deferLoad = false } = {}) {
  ...
  const iframe = {
    ...
    addEventListener: (_: "load", callback: () => void) => {
  ...
  const environment: SandboxEnvironment = {
    createIframe: () => iframe,
    appendIframe: () => {
      if (!deferLoad) loadListener?.();
    },
    addMessageListener: (callback) => {
      messageListener = callback;
    },
    removeMessageListener: (callback) => {
      if (messageListener === callback) messageListener = undefined;
    },
  };
  return {
    environment,
...
test("checks iframe source and run token before delivering frames", () => {
  ...
  testbed.emit(testbed.iframe.contentWindow, {
    kind: "frame",
    token: "stale",
    commands: [{ op: "clear", color: 0 }],
  });
...
test("delivers validated audio requests", () => {
  ...
  const run = new SandboxRun(
    compiled,
    { onAudio: (command) => audio.push(command) },
    testbed.environment,
    "0123456789abcdef0123456789abcdef",
  );
  run.start();
