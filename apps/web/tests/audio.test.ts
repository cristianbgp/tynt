...
test("schedules a bounded oscillator and stops active cartridge audio", async () => {
  ...
  const oscillator = {
    type: "sine",
    frequency: { setValueAtTime: (value: number) => events.push(value) },
    connect: () => {},
    start: (time: number) => events.push(`start:${time}`),
    stop: (time?: number) => events.push(`stop:${time}`),
    addEventListener: () => {},
    disconnect: () => {},
  };
  const gain = {
    gain: {
      setValueAtTime: (value: number) => events.push(value),
