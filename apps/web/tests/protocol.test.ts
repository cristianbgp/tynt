...
describe("sandbox protocol", () => {
  ...
  test("accepts valid iframe messages and rejects invalid numeric data", () => {
    expect(
      ...
      kind: "frame",
      token,
      commands: [{ op: "pixel", x: 1, y: 2, color: 3 }],
    });
    expect(() =>
      parseIframeMessage(
        { kind: "frame", token, commands: [{ op: "pixel", x: NaN, y: 2, color: 3 }] },
        token,
      ),
    ).toThrow(/command/i);
    expect(() =>
      parseIframeMessage(
        { kind: "frame", token, commands: [{ op: "pixel", x: Infinity, y: 2, color: 3 }] },
        token,
      ),
    ).toThrow(/command/i);
  });
  ...
  test("validates lifecycle phase and normalized errors", () => {
    expect(
      parseIframeMessage({ kind: "phase", token, phase: "update", state: "begin" }, token).kind,
    ).toBe("phase");
    expect(
      parseIframeMessage(
        { kind: "error", token, phase: "draw", message: "boom", line: 8, column: 2 },
        token,
      ).kind,
    ).toBe("error");
    expect(() =>
      parseIframeMessage({ kind: "error", token, phase: "internal", message: "boom" }, token),
    ).toThrow(/phase/i);
  });
  ...
  test("enforces command count and text code-point limits", () => {
    const atLimit = Array.from({ length: MAX_COMMANDS }, () => ({
      op: "clear" as const,
      color: 0,
    }));
    ...
    expect(() =>
      validateFrame([{ op: "text", value: "🎮".repeat(1025), x: 0, y: 0, color: 1 }]),
    ).toThrow(/1,024/);
  });
  ...
  test("bounds sprite and map rendering work", () => {
    expect(
      validateFrame([
        { op: "sprite", pixels: [1, 2, 3, 0], width: 2, height: 2, x: 0, y: 0, transparent: 0 },
      ]),
    ).toHaveLength(1);
    expect(() =>
      validateFrame([
        { op: "sprite", pixels: [1], width: 160, height: 144, x: 0, y: 0, transparent: 0 },
      ]),
    ).toThrow(/sprite/i);
    expect(() =>
      validateFrame([
        {
          op: "map",
          tiles: Array(1025).fill(0),
          columns: 1,
          tileWidth: 8,
          tileHeight: 8,
          spritesheet: Array(64).fill(1),
          sheetColumns: 1,
          x: 0,
          y: 0,
          transparent: 0,
  ...
  test("counts encoded UTF-8 and enforces the frame byte limit", () => {
    ...
    const oversized = Array.from({ length: 1024 }, () => ({
      op: "text" as const,
      value: "x".repeat(1024),
      x: 0,
      y: 0,
    ...
    expect(() =>
      parseIframeMessage(
        { kind: "frame", token, commands: [], ignored: "x".repeat(MAX_FRAME_BYTES) },
        token,
      ),
    ).toThrow(/1 MiB/);
  });
