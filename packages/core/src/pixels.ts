...
const FONT: Record<string, string> = {
  " ": "000000000000000",
  "?": "110001010000010",
  "!": "010010010000010", "\"": "101101000000000", "#": "101111101111101", "$": "011110010011110",
  "%": "101001010100101", "&": "010101010101011", "'": "010010000000000", "(": "001010010010001",
  ")": "100010010010100", "*": "000101010101000", "+": "000010111010000", ",": "000000000010100",
  "/": "001001010100100", ";": "000010000010100", "<": "001010100010001", "=": "000111000111000",
  ">": "100010001010100", "@": "111101111100111", "[": "110100100100110", "\\": "100100010001001",
  "]": "011001001001011", "^": "010101000000000", "`": "100010000000000", "{": "011010110010011",
  "|": "010010010010010", "}": "110010011010110", "~": "000011110000000",
  ...
  "4": "101101111001001", "5": "111100110001110", "6": "011100110101010", "7": "111001010100100",
  "8": "010101010101010", "9": "010101011001110",
  ".": "000000000000010", ":": "000010000010000", "-": "000000111000000", "_": "000000000000111",
};
...
function drawLine(surface: PixelSurface, x0Value: number, y0Value: number, x1Value: number, y1Value: number, color: number): void {
...
function drawRect(surface: PixelSurface, command: Extract<DrawCommand, { op: "rect" }>): void {
  ...
  drawLine(surface, x, y + height - 1, x + width - 1, y + height - 1, command.color);
  drawLine(surface, x, y, x, y + height - 1, command.color);
  drawLine(surface, x + width - 1, y, x + width - 1, y + height - 1, command.color);
}

function circlePoints(surface: PixelSurface, cx: number, cy: number, x: number, y: number, color: number): void {
  const points = [[cx+x,cy+y],[cx-x,cy+y],[cx+x,cy-y],[cx-x,cy-y],[cx+y,cy+x],[cx-y,cy+x],[cx+y,cy-x],[cx-y,cy-x]];
  for (const [px, py] of points) surface.set(px, py, color);
}

function drawCircle(surface: PixelSurface, command: Extract<DrawCommand, { op: "circle" }>): void {
  ...
  if (command.fill) {
    for (let y = -radius; y <= radius; y++) {
      ...
      for (let x = -span; x <= span; x++) surface.set(cx + x, cy + y, command.color);
    }
    return;
  }
  let x = radius;
...
function drawText(surface: PixelSurface, command: Extract<DrawCommand, { op: "text" }>): void {
  if ([...command.value].length > 1024) throw new Error("text() accepts at most 1,024 characters");
  const startX = roundCoordinate(command.x);
  let x = startX;
  let y = roundCoordinate(command.y);
  for (const rawCharacter of command.value) {
    if (rawCharacter === "\n") { x = startX; y += 6; continue; }
    const glyph = FONT[rawCharacter.toUpperCase()] ?? FONT["?"];
    for (let index = 0; index < glyph.length; index++) {
      if (glyph[index] === "1") surface.set(x + (index % 3), y + Math.floor(index / 3), command.color);
    }
    x += 4;
  }
}

function drawSprite(surface: PixelSurface, command: Extract<DrawCommand, { op: "sprite" }>): void {
...
function drawMap(surface: PixelSurface, command: Extract<DrawCommand, { op: "map" }>): void {
  ...
  command.tiles.forEach((rawTile, index) => {
    ...
    for (let py = 0; py < tileHeight; py++) {
      for (let px = 0; px < tileWidth; px++) pixels.push(command.spritesheet[(sourceY + py) * sheetWidth + sourceX + px] ?? command.transparent);
    }
    drawSprite(surface, {
      op: "sprite", pixels, width: tileWidth, height: tileHeight,
...
export function applyCommand(surface: PixelSurface, command: DrawCommand): void {
  switch (command.op) {
    case "clear": surface.clear(command.color); break;
    case "pixel": surface.set(command.x, command.y, command.color); break;
    case "line": drawLine(surface, command.x0, command.y0, command.x1, command.y1, command.color); break;
    case "rect": drawRect(surface, command); break;
    case "circle": drawCircle(surface, command); break;
    case "text": drawText(surface, command); break;
    case "sprite": drawSprite(surface, command); break;
    case "map": drawMap(surface, command); break;
  }
}
